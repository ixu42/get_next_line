...
/*                                                    +:+ +:+         +:+     */
/*   By: ixu <ixu@student.hive>                     +#+  +:+       +#+        */
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/15 10:25:05 by ixu               #+#    #+#             */
/*   Updated: 2023/11/15 10:25:11 by ixu              ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static int	create_list(t_list **list, int fd)
{
	...
	while (!has_newline_char(list[fd]))
	{
		...
		buf[chars_read] = '\0';
		status = ft_lstadd_back(&list[fd], buf);
		if (status == -2)
		{
			free(buf);
			return (-2);
